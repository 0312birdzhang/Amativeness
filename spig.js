...
jQuery(document).ready(function ($) {
    $("#spig").mousedown(function (e) {
        if(e.which==3){
        showMessage("秘密通道:<br />    <a href=\"http://www.anotherhome.net\" title=\"首页\">首页</a>    <a href=\"http://www.anotherhome.net/wp-admin\" title=\"后台\">后台</a>",10000);
}
});
...
jQuery(document).ready(function ($) {
    if (isindex) { //如果是主页
        ...
        showMessage('欢迎来到BirdZhang的博客 ' + title + ' ', 6000);
    }
    $(".spig").animate({
...
jQuery(document).ready(function ($) {
    window.setInterval(function () {
        msgs = ["去留言板申请友链吧！", "乾坤大挪移！", "我飘过来了！~", "我飘过去了", "我得意地飘！~飘！~"];
        var i = Math.floor(Math.random() * msgs.length);
        s = [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75];
    ...
    $(".mumu").click(function () {
        if (!ismove) {
            ...
            if (stat_click > 4) {
                ...
                msgs = ["筋斗云！~我飞！", "我跑呀跑呀跑！~~", "别摸我，非礼啦！", "惹不起你，我还躲不起你么？", "不要摸我了，我会告诉主人来打你的！", "干嘛动我呀！小心我咬你！"];
                var i = Math.floor(Math.random() * msgs.length);
                //showMessage(msgs[i]);
            }
        s = [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75];
